...
class FurnitureRecommendationSystem:
    ...
    #     recommended_items = []
    #     # Assuming self.dataset is your DataFrame containing 'title' and 'categories'
    #     self.dataset['combined'] = self.dataset['title'].fillna('') + ' ' + self.dataset['categories'].fillna('')
    #     for category in furniture_categories:
    #         # category_items = self.dataset[self.dataset['title'].str.contains(category, case=False)]
    #         category_items = self.dataset[self.dataset['combined'].str.contains(category, case=False)]
    #         if not category_items.empty:
    #             for _, item in category_items.iterrows():
    #                 # combined_details = item['details'] + " " + item['description']
    #                 # similarity_score = self.calculate_similarity(room_analysis['average_color'], combined_details)
    ...
    #                 print(color_similarity,description_similarity)

    #                 # Aggregate similarity score
    #                 similarity_score = (color_similarity + description_similarity) / 2
    #                 recommended_items.append({'Title': item['title'], 'Details': item['details'], 'Similarity': similarity_score})
...
if __name__ == "__main__":
    ...
    furniture_categories = ['chair','table']  # Example categories
