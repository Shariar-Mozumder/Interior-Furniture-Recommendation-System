...
# Function to generate color variations based on weights
def generate_color_variations(base_colors, weights, num_variations=100):
    variations = []
    base_rgb = [hex_to_rgb(color) for color in base_colors]
    
    for _ in range(num_variations):
        # Generate new colors by weighting the base colors
        weighted_color = np.zeros(3)
        for idx, color in enumerate(base_rgb):
            weight = weights[idx]
            weighted_color += np.array(color) * weight
        
        # Ensure the generated color is within RGB bounds
        weighted_color = np.clip(weighted_color, 0, 1)
        variations.append(rgb_to_hex(weighted_color))
...
def find_compatible_colors(hex_colors, weights_csv_path, num_compatible_colors=3):
    ...
    # Extract weights from the input colors
    weights = np.array([weights_df.loc[f'color-{i+1}'].values[0] for i in range(len(hex_colors))])
    
    # Generate color variations
    color_variations = generate_color_variations(hex_colors, weights, num_variations=50)
